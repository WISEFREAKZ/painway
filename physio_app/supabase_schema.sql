...
--
-- MEDIA SOURCE: every media_url below points to a real, live image file
-- hosted on GitHub's raw CDN from `free-exercise-db`
-- (https://github.com/yuhonas/free-exercise-db) — an open, Unlicense
-- (public-domain) exercise dataset. Every single URL in this file was
-- individually HTTP-verified (200 OK) before being included here — none
-- are placeholders or guessed IDs.
--
-- WHY STATIC IMAGES, NOT ANIMATED GIFS: true animated GIF demonstrations
-- for these specific therapeutic exercise variations are not available
-- from a verifiably open-license source. The datasets that do offer
-- exercise GIFs (e.g. ExerciseDB-derived repos) are repackaging content
-- from a commercial, closed API — their MIT-license claims on that
-- media are legally questionable, since a repo can't relicense content
-- it doesn't itself own. free-exercise-db's Unlicense status is
-- genuinely public domain and directly confirmed by its own repo, so
-- it's the safer choice despite being static images rather than GIFs.
-- The app's Image.network() calls work identically either way if you
-- later find or license real GIFs and swap the media_url values.
--
...
begin
  ...
  insert into public.exercises
    (category_id, title, target_muscles, description, steps, media_url, duration_seconds)
  values
    (
      foot_heel_cat_id,
      'Ankle Circles',
